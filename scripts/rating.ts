...
function newTrueskillEnv() {
  ...
  const mu = 5000; //initial mean of ratings
  const sigma = mu / 3; //initial standard deviation of ratings
  const beta = sigma / 2; //distance that guarantees about 76% chance of winning
...
function testrating() {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    console.log(i);
    console.log(`winner rating: ${w.exposed.toFixed(0)} (${w.mu.toFixed(0)}, ${w.sigma.toFixed(0)})`);
    console.log(`loser rating: ${l.exposed.toFixed(0)} (${l.mu.toFixed(0)}, ${l.sigma.toFixed(0)})`);
    console.log();
